...
public class TopDocs implements java.io.Serializable {
  ...
  TopDocs(int totalHits, ScoreDoc[] scoreDocs) {
    this(totalHits, scoreDocs, Float.NaN, 0.0f, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
  }

  public TopDocs(int totalHits, ScoreDoc[] scoreDocs, float maxScore, double sum, double max, double min) {
  ...
  public static TopDocs merge(Sort sort, int topN, TopDocs[] shardHits) throws IOException {
    ...
    for(int shardIDX=0;shardIDX<shardHits.length;shardIDX++) {
      ...
      if (shard.scoreDocs != null && shard.scoreDocs.length > 0) {
        ...
        sum += shard.getSum();
        max = Math.max(max, shard.getMax());
        min = Math.min(min, shard.getMin());
    ...
    if (sort == null) {
      return new TopDocs(totalHitCount, hits, maxScore, sum, max, min);
    } else {
      return new TopFieldDocs(totalHitCount, hits, sort.getSort(), maxScore, sum, max, min);
    }
  }
}
