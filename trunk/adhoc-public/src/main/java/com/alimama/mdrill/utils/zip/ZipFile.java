...
public class ZipFile {
    ...
    long length=0;
    public ZipFile(FSDataInputStream f,long len, String encoding, boolean useUnicodeExtraFields)
        throws IOException {
    	this.length=len;
    ...
    private void positionAtCentralDirectory()
        ...
        boolean searchedForZip64EOCD =
            archive.getPos() > ZIP64_EOCDL_LENGTH;
        if (searchedForZip64EOCD) {
            archive.seek(archive.getPos() - ZIP64_EOCDL_LENGTH);
            archive.readFully(WORD_BUF);
    ...
    private boolean tryToLocateSignature(long minDistanceFromEnd,
        ...
        final long stopSearching =
            Math.max(0L,this.length - maxDistanceFromEnd);
        if (off >= 0) {
