...
public class MysqlCallback implements IOffilneDownloadCallBack{
	...
	public void finish() {
			...
			for(String s:cols[1].split(","))
			{
				...
				try {
					...
					if(!this.isfail)
					{
						...
						contentw.append("下载地址(七星阵):"+strurl+"<br>");
					}else{
						contentw.append(" <br>");
		...
		if(!this.isfail)
		{
			...
			content.append("<tr><td>下载地址(七星阵)：<a href=\"http://adhoc.etao.com:9999/downloadoffline.jsp?uuid="+this.getUuid()+"\">http://adhoc.etao.com:9999/downloadoffline.jsp?uuid="+this.getUuid()+"</a></tr></td>\r\n");
		}else{
			content.append("<tr><td>任务名称："+this.jobname+"</tr></td>\r\n");
