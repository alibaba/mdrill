...
public abstract class IndexReader implements Cloneable,Closeable {
  ...
  public TermDocs termDocs(Term term,int buffer) throws IOException {
    ensureOpen();
    TermDocs termDocs = termDocs(buffer);
    termDocs.seek(term);
    return termDocs;
  }
  
  public TermDocs termDocs(Term term) throws IOException {
	   return termDocs(term, -1);
	  }
   ...
   * @throws IOException if there is a low-level IO error
   */
  public abstract TermDocs termDocs(int buffer) throws IOException;
  ...
  public abstract String getStringCacheKey();
