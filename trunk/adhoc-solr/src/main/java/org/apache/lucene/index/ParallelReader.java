...
public class ParallelReader extends IndexReader {
  ...
  */
  ...
  public String getStringCacheKey(){
	  ...
	  for(IndexReader r:this.readers){
  ...
  */
  ...
  @Override
  public TermDocs termDocs(Term term,int buffer) throws IOException {
    ensureOpen();
    return new ParallelTermDocs(term,buffer);
  }

  @Override
  public TermDocs termDocs(int buffer) throws IOException {
    ensureOpen();
    return new ParallelTermDocs(buffer);
  }

  @Override
  public TermPositions termPositions(Term term) throws IOException {
    ensureOpen();
    return new ParallelTermPositions(term,-1);
  }

  @Override
  public TermPositions termPositions() throws IOException {
    ensureOpen();
    return new ParallelTermPositions(-1);
  }
  ...
  private class ParallelTermDocs implements TermDocs {
    ...
    public ParallelTermDocs(Term term,int buffer) throws IOException {
    	this.buffer=buffer;
      if (term == null)
        termDocs = readers.isEmpty() ? null : readers.get(0).termDocs(null,this.buffer);
      else
        seek(term);
    }
    ...
    public void seek(Term term) throws IOException {
      IndexReader reader = fieldToReader.get(term.field());
      termDocs = reader!=null ? reader.termDocs(term,this.buffer) : null;
    }
  ...
  private class ParallelTermPositions
    ...
    public ParallelTermPositions(int buffer) {
    	super(buffer);
    }
    public ParallelTermPositions(Term term,int buffer) throws IOException { 
    	super(buffer);
    	seek(term); }
