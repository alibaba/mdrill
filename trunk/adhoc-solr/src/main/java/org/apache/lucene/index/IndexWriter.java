...
public class IndexWriter implements Closeable, TwoPhaseCommit {
  ...
  */
  ...
  private void copySegmentIntoCFS(SegmentInfo info, String segName) throws IOException {
    ...
    String segFileNamepos = IndexFileNames.segmentFileName(segName, IndexFileNames.COMPOUND_FILE_EXTENSION_POS);

    Collection<String> files = info.files();
    CompoundFileWriter cfsWriter = new CompoundFileWriter(directory, segFileName,segFileNamepos);
  ...
  final private int mergeMiddle(MergePolicy.OneMerge merge) 
    ...
    try {
      ...
      if (useCompoundFile) {
        ...
        final String compoundFileNamepos = IndexFileNames.segmentFileName(mergedName, IndexFileNames.COMPOUND_FILE_EXTENSION_POS);

        try {
          if (infoStream != null) {
            message("create compound file " + compoundFileName);
          }
          merger.createCompoundFile(compoundFileName,compoundFileNamepos, merge.info);
          success = true;
