...
<h2><a style="color:#551890" href="./siteoverlap.jsp">重合度</a>/<a style="color:#551890" href="./crowd.jsp">人群覆盖</a>/流量效果</h2>
...
<td>
	...
	<select name="sourceform" id="sourceform"  onchange="tooglemode()">
      <option value="site"  selected="selected">流量分人群效果</option>
      <option value="pid">流量分DSP效果</option>
  </select>
  
  网站：
	
	<input type="text" name="match_domain" id="match_domain"  value="qq.com" />
 
   pid:
	
	<input type="text" name="match_pid" id="match_pid"  value="mm_43679981_5596418_17474382" />
...
<input type="button" value="更多筛选条件..." onclick="show_matchstr()" />
	  <div id="matchstr" style="display:none">
...
var today=new Date();
jQuery("#thedateStart").val(parseDay(new Date(today.getTime()-1000*3600*24*3)));
...
$("#tablelist_id").html('<table id="list4"  dir="ltr" style="width: 2000px;"></table><div id="pager2"></div>');
	...
	showload();
				...
				var newq=[];
				  newq.push({"thedate":{"operate":1,"value":[thedatestart]}});
	  if($("#match_domain").val()!="*")
	  {
	  newq.push({"domain":{"operate":1,"value":[$("#match_domain").val()]}});
	}
	if($("#match_pid").val()!="*")
	{
	  newq.push({"pid":{"operate":1,"value":[$("#match_pid").val()]}});
	}

	var filter_arr=[];
	if($("#sourceform").val()=="site")
	{
			filter_arr=g_site_filter.split(",");
		
	}
	else
		{
			filter_arr=g_dsp_filter.split(",");
		}
	...
	if($("#match_pid").val()!="*")
	{
	  newq.push({"pid":{"operate":1,"value":[$("#match_pid").val()]}});
	  	...
	  	var requestparams={};
			...
			requestparams.project="rpt_adpmp_tanx_dsp_effect_d";
			//requestparams.order="desc";
			//requestparams.sort="sum(uv)";
			//requestparams.groupby="";
			requestparams.fl=g_dspfl;
		  requestparams.q=JSON.stringify(newq);
			...
			$.post("/result.jsp",requestparams,
					function (data, textStatus){	
						...
						showresultable(returnresult,requestparams.fl);
	  	...
	  	var requestparams={};
			requestparams.start=0;
			requestparams.rows=10000;
			requestparams.project="fact_adpmp_tanx_crowd_effect_d";
			//requestparams.order="desc";
			//requestparams.sort="sum(uv)";
			//requestparams.groupby="thedate,pid,user_tag_value";
			requestparams.fl=g_sitefl;
		  requestparams.q=JSON.stringify(newq);
...
var g_dspfl="thedate,domain,pid,pub_id,pub_name,site_id,site_name,adzone_id,adzone_name,dsp_id,dsp_name,adzone_rec_pv,log_x_ss_eff_pv,x_eff_click,log_x_ss_eff_charge,ctr,cpm,pv,click,charge,alipay_amt,alipay_cnt,roi,cvr";

var g_sitefl="thedate,domain,pid,pub_id,pub_name,site_id,site_name,adzone_id,adzone_name,user_tag,user_tag_value,pv,uv,click,click_uv,charge,alipay_amt,alipay_cnt,alipay_uv";


var g_dsp_filter="pub_id,pub_name,site_id,site_name,adzone_id,adzone_name,dsp_id,dsp_name,adzone_rec_pv,log_x_ss_eff_pv,x_eff_click,log_x_ss_eff_charge,ctr,cpm,pv,click,charge,alipay_amt,alipay_cnt,roi,cvr";

var g_site_filter="pub_id,pub_name,site_id,site_name,adzone_id,adzone_name,user_tag,user_tag_value,pv,uv,click,click_uv,charge,alipay_amt,alipay_cnt,alipay_uv";


function showresultable(resultlist,fl)
{	
				...
				{
					colname.push(g_colname_kv[cols[i]]?g_colname_kv[cols[i]]:cols[i]);
					if(!g_coltype_kv[cols[i]]||g_coltype_kv[cols[i]]=="string")
					{
											colmode.push({name:cols[i],index:cols[i], width:100,searchoptions:{"clearSearch":false,sopt:['eq']}}); 

					}else{
											colmode.push({name:cols[i],index:cols[i],  width:100, align:"right",sorttype:"float",searchoptions:{clearSearch:false,sopt:['eq','ne','le','lt','gt','ge']}});
...
var colname1={
      ...
      "data": [{
            ...
            "group_member": [{
                  ...
                  "descript": "接入tanx的PV数",
                  "calc": "1,2,3,4",
                  "operate": "sum,max,min,average,count",
                  "is_dim": "0",
                  "unit": "",
                  "unit_pos": "",
                  "mufactor": "",
...
var colname2={
      ...
      "data": [{
            ...
            "group_member": [{
                  ...
                  "unit": "",
                  "unit_pos": "",
                  "mufactor": "sum,max,min,average,count",
                  "thousands": "1",
                  "decimal": ""
            }, {
                  "name": "uv数",
...
var g_colNamelist=[colname1,colname2];
...
{
	var subdata=g_colNamelist[i];
	for(var j=0;j<subdata.data.length;j++)
	{
			var subdata2=subdata.data[j].group_member;

		for(var k=0;k<subdata2.length;k++)
	{
		var subdata3=subdata2[k];
		g_colname_kv[subdata3.key]=subdata3.name;
		g_coltype_kv[subdata3.key]=subdata3.type;
	}
