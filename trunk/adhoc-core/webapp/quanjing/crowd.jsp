...
<h2><a style="color:#551890" href="./siteoverlap.jsp">重合度</a>/人群覆盖/<a style="color:#551890" href="./effect.jsp">流量效果</a></h2>
...
<td>
	...
	<select name="sourceform" id="sourceform"  onchange="tooglemode()">
      <option value="site"  selected="selected">网站</option>
      <option value="pid">pid</option>
  </select>
  
  
	<input type="text" name="match_str" id="match_str"  value="qq.com" />
  
  选择人群：
   <select name="attr_site" id="attr_site" >
      <option value="性别"  selected="selected">性别</option>
      <option value="地域">地域</option>
       <option value="不限">不限</option>
       <option value="年龄段">年龄段</option>
       <option value="用户等级">用户等级</option>
      
  </select>
  
 <select name="attr_pid" id="attr_pid"  style="display:none">
      <option value="性别"  selected="selected">性别</option>
      <option value="职业">职业</option>
      <option value="地域">地域</option>
      <option value="年龄段">年龄段</option>
      <option value="月消费金额">月消费金额</option>
      <option value="用户等级">用户等级</option>
      <option value="不限">不限</option>
      <option value="用户长期喜好购买的一级类目"> 用户长期喜好购买的一级类目 </option>
      <option value="短期用户浏览一级类目"> 短期用户浏览一级类目 </option>
      <option value="短期用户搜索一级类目偏好"> 短期用户搜索一级类目偏好 </option>
       
      <option value="用户长期收藏购买的一级类目"> 用户长期收藏购买的一级类目 </option>
...
<td >
					...
					<td colspan=2 align=center> <font color=red size=3>注：如果涉及多个网站|pid，则会对网站和pid，按照属性值汇总</font>
							</td>
				</tr>
					...
					<td>
					 ...
					 <div id="cate_placeholder_pie" style="width: 400px;height: 300px;">	</div>
					</td>
					<td>
					
					 		<div id="cate_placeholder" style="width: 400px;height: 300px;">	</div>
					</td>
				</tr>
				</table>
...
$("#tablelist_id").html('<table id="list4"  dir="ltr" style="width: 2000px;"></table><div id="pager2"></div>');
	...
	if($("#sourceform").val()=="site")
	{
	  ...
	  newq.push({"domain":{"operate":1,"value":[$("#match_str").val()]}});
	  newq.push({"user_tag":{"operate":1,"value":[$("#attr_site").val()]}});
	  	var requestparams={};
			requestparams.start=0;
			requestparams.rows=10000;
			requestparams.project="fact_adpmp_tanx_domain_crowd_uv_d";
			requestparams.order="desc";
			requestparams.sort="sum(uv)";
			requestparams.groupby="thedate,domain,user_tag_value";
			requestparams.fl="thedate,domain,user_tag_value,sum(uv)";
					...
					function (data, textStatus){	
					  ...
					  var returnMap={};
						...
						for(var i=0;i<listtmp.length;i++)
						{
							...
							item["user_tag_value"]=item["user_tag_value"];
						  item["uv"]=(parseFloat(item["sum(uv)"])).toFixed(2);
							returnresult.push(item);
							
							if(!returnMap[item["user_tag_value"]])
							{
								returnMap[item["user_tag_value"]]=parseInt(item["uv"]);
							}else{
								
								returnMap[item["user_tag_value"]]=parseInt(returnMap[item["user_tag_value"]])+parseInt(item["uv"]);
							}
						}
		...
		var newq=[];
	  newq.push({"thedate":{"operate":1,"value":[thedatestart]}});
	  newq.push({"pid":{"operate":1,"value":[$("#match_str").val()]}});
	  newq.push({"user_tag":{"operate":1,"value":[$("#attr_pid").val()]}});
	  	var requestparams={};
			requestparams.start=0;
			requestparams.rows=10000;
			...
			requestparams.fl="thedate,pid,user_tag_value,sum(uv)";
		  requestparams.q=JSON.stringify(newq);
...
function showpie2(mapdata)
{
							...
							{
								newchardata.push({"label":p,"data":mapdata[p]});
							}
            
            	var placeholder = $("#cate_placeholder_pie");
							 placeholder.unbind();
                  placeholder.empty();
								$.plot('#cate_placeholder_pie', newchardata, {
								    series: {
								        pie: {
								            show: true,
								            radius: 1,
...
function showpie(mapdata)
{
							...
							for(var p in mapdata)
							{
								newchardata.push([p,mapdata[p]]);
								newcharlabel.push(p);
							}
							var placeholder = $("#cate_placeholder");
							placeholder.empty();
							placeholder.append("<div id='cate_placeholder_1'></div>");
							
									var placeholder = $("#cate_placeholder_1");
									placeholder.unbind();
                  placeholder.empty();

						

									$("#cate_placeholder_1").jqChart({
                title: "",
...
{	
	...
	datastr:resultliststr,
	 height: 450,
	 colNames:["thedate","网站|pid","属性值","uv"], 
	 colModel:[ 
	  {name:'thedate',index:'thedate', width:80,searchoptions:{"clearSearch":false,sopt:['eq']}}, 
	  {name:'domain',index:'domain', width:100,searchoptions:{"clearSearch":false,sopt:['eq']}}, 
	  {name:'user_tag_value',index:'user_tag_value', width:100,searchoptions:{"clearSearch":false,sopt:['eq']}}, 
	  {name:'uv',index:'uv',  width:100, align:"right",sorttype:"float",searchoptions:{clearSearch:false,sopt:['eq','ne','le','lt','gt','ge']}}
