...
public static String addjson(String json) throws Exception
	{
		...
		buff.append("<input type=\"text\" name=\"role\" value=\""+String.valueOf(jsonObj.getInt("role"))+"\" />");
...
<%
	...
	if(checkpwd.equals(getMD5(checkuser+"1qazxcvfr432wsde"))&&(checkuser.equals("yannian.mu")||checkuser.equals("zhangzhuang.sbb")))
	{
	...
	if(tp.equals("add"))
	{
		...
		out.println(UserJson.add(mapval.get("userid"),mapval));
	}
	
	if(tp.equals("jsonlist"))
	{
		UserJson.addJsonList(String.valueOf(request.getParameter("jsonlist")).trim());
	}
	
	if(tp.equals("del"))
	{
		out.println(UserJson.del(mapval.get("userid")));
	}
